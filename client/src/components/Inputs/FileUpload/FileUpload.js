...
const FileUpload = ({ handleUpload }) => {
  ...
  const onDrop = async uploadedFiles => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/jobs/uploadimage', formData, config);
  ...
  const onRemove = async id => {
    await axios.get(`http://localhost:5000/api/jobs/removeimage?public_id=${id}`);
